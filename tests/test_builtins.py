...
class BuiltinTest(test_common.ClusterTest):
  def test_arange(self):
    ...
    Assert.all_eq(spartan.arange((10, ), start=-1).glom(), np.arange(-1, 9))
    Assert.all_eq(spartan.arange((10, ), start=1).glom(), np.arange(1, 11))
    Assert.all_eq(
        spartan.arange((3, 5), start=-1).glom(),
    ...
    Assert.all_eq(
        spartan.arange((10, ), start=-1, step=2).glom(),
        np.arange(-1, 19, 2))
    ...
    Assert.all_eq(
        spartan.arange((3, 5), start=1, step=2).glom(),
