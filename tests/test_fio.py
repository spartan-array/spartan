...
class TestFIO(test_common.ClusterTest):
  def test1(self):
    ...
    Assert.eq(expr.save(t1, "__fiotest1", '.', True), True)
    Assert.all_eq(t1.glom(), expr.load("__fiotest1", '.', True).glom())
    Assert.eq(expr.pickle(t1, "__fiotest2", '.', False), True)
    Assert.all_eq(t1.glom(), expr.unpickle("__fiotest2", '.', False).glom())
    Assert.eq(expr.pickle(t1, "__fiotest2", '.', True), True)
