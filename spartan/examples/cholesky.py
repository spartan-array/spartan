...
def _cholesky_dsyrk_dgemm_mapper(input, with_array, with_ex, k):
  ...
  if with_ex.ul[0] == with_ex.ul[1] and with_ex.lr[0] == with_ex.lr[1]:
    # diag block
...
def cholesky(A):
  ...
  for k in range(n):
    ...
    A = expr.region_map(A, col_exs, _cholesky_dsyrk_dgemm_mapper, fn_kw=dict(with_array=True, with_ex=True, k=k))
