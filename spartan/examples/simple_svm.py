...
class SVM:
  ...
  def predict(self, data):
    ...
    margs = self.margins(data).evaluate()
  ...
  def take_step(self, i, j, N, labels, kernel_results):
    ...
    for i in xrange(N):
      self.E[i, 0] = self.b + expr.reduce(self.alpha, axis=None,
                                          dtype_fn=lambda input: input.dtype,
                                          local_reduce_fn=margin_mapper,
                                          accumulate_fn=np.add,
                                          fn_kw=dict(label=labels,
                                                     data=kernel_results[:, i].evaluate())).glom() - labels[i, 0]
    #print 'b', self.b
  ...
  def train_smo_1998(self, data, labels):
    ...
    self.b = 0.0
    self.alpha = expr.zeros((N, 1), dtype=np.float64,
    ...
    self.E = expr.zeros((N, 1), dtype=np.float64,
                        tile_hint=[N / self.ctx.num_workers, 1]).evaluate()
    for i in xrange(N):
      self.E[i, 0] = self.b + expr.reduce(self.alpha, axis=None,
                                          dtype_fn=lambda input: input.dtype,
    ...
    for i in xrange(D):
      self.w[i, 0] = expr.reduce(self.alpha, axis=None,
                                 dtype_fn=lambda input: input.dtype,
                                 local_reduce_fn=margin_mapper,
                                 accumulate_fn=np.add,
                                 fn_kw=dict(label=labels,
                                            data=expr.evaluate(data[:, i]))).glom()
    self.usew_ = True
  ...
  def train_smo_2005(self, data, labels):
    ...
    self.b = 0.0
    alpha = expr.zeros((N, 1), dtype=np.float64,
                       tile_hint=[N / self.ctx.num_workers, 1]).evaluate()

    # linear kernel
    kernel_results = expr.dot(data, expr.transpose(data),
                              tile_hint=[N / self.ctx.num_workers, N])
    gradient = expr.ones((N, 1), dtype=np.float64,
                         tile_hint=[N / self.ctx.num_workers, 1]) * -1.0
    ...
    for i in xrange(D):
      self.w[i, 0] = expr.reduce(alpha, axis=None, dtype_fn=lambda input: input.dtype,
                                 local_reduce_fn=margin_mapper,
                                 accumulate_fn=np.add,
                                 fn_kw=dict(label=labels, data=expr.evaluate(data[:, i]))).glom()
