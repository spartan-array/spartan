...
def retile(array, tile_hint):
  ...
  new_array = shuffle(ndarray(array.shape, tile_hint=tile_hint).force(), _retile_mapper, kw={'orig_array':array}, shape_hint=array.shape, 
                      cost_hint={hash(array):{'%d%d' % (tiling_type, tiling_type): 0, '%d%d' % (1-tiling_type, tiling_type): np.prod(array.shape)}})
