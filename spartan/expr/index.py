...
These are generated by a __getitem__ call to an `Expr` 
(e.g. x[0:10, 20:30] returns an `IndexExpr`).  

Theoretically we could determine at whether something
is a slice (indexing by a tuple) versus a boolean/index array
at the call site, but for the moment these are both managed 
by the `IndexExpr`.
'''
import numpy as np
