...
public:
    ...
    unsigned long py_get(TileId* tile_id, SubSlice* subslice, GetResp* resp) {
        GetReq req(*tile_id, *subslice);
        return (unsigned long)_py_send<GetReq, GetResp>(tile_id->worker, &CWorker::get,
                                         &spartan::WorkerProxy::async_get, req, resp);
    }

    rpc::Future* get_flatten(const TileId& tile_id, const SubSlice& subslice, GetResp* resp,
            bool wait=true, int32_t timeout=CBlobCtx::RPC_TIMEOUT) {
        GetReq req(tile_id, subslice);
    ...
    unsigned long py_get_flatten(TileId* tile_id, SubSlice* subslice, GetResp* resp) {
        GetReq req(*tile_id, *subslice);
    ...
    rpc::Future* update(const TileId& tile_id, const SubSlice& region, std::string& data,
    ...
    unsigned long py_update(TileId* tile_id, SubSlice* region, std::string* data,
                        int reducer) {
        UpdateReq req(*tile_id, *region, *data, reducer);
