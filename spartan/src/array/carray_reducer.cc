...
slice_dense_outer_loop(CArray *ip1, CArray *ip1_state, CArray *ip2, CExtent *ex, REDUCER reducer)
{
    ...
    if (last_sliced_dim == ip1->get_nd() - 1) {
        continous_size = ex->lr[last_sliced_dim] - ex->ul[last_sliced_dim];
    } else {
        continous_size = 1;
    ...
    do {
        ...
        for (i = last_sliced_dim; i >= 0; i--) {
            if (last_sliced_dim == ip1->get_nd() - 1 && i == last_sliced_dim) {
                curr_idx[i] = ex->ul[i];
                continue;
            } else {
                curr_idx[i] += 1;
...
void
full_dense_outer_loop(CArray *ip1, CArray *ip1_state, CArray *ip2, REDUCER reducer)
{
    char *arrays[3] = {ip1->get_data(), ip1_state->get_data(), ip2->get_data()};
    npy_intp inner_steps[1] = {ip1->get_strides()[ip1->get_nd() - 1]};
    npy_intp size = 1;
...
/* This must be sync with REDUCER enumeration */
/* Only support add and or now. These two are full to implement */
